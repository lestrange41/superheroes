            ...
            <div style={{ backgroundImage: `url(${img1})`, backgroundSize: 'cover', backgroundPosition: 'center', minHeight: '100vh' }}>
                ...
                <div className='container mx-auto py-8'>
                    ...
                    <div className='grid grid-cols-3 gap-4 mt-8'>
                        {filteredHeroes.map((hero) => (
                            ...
                            <h2 className='text-3xl text-primary font-semibold mb-4'>{hero.name.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}</h2>
                                <p className='text-white-600 mb-2'><strong>Nombre real:</strong> {hero.realName.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p className='text-white-600 mb-2'><strong>Tipo:</strong> {hero.type}</p>
                                <p className='text-white-600 mb-2'><strong>Poderes:</strong> {hero.powers.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p className='text-white-600 mb-2'><strong>Edad:</strong> {hero.age}</p>
                                <p className='text-white-600 mb-2'><strong>Raza:</strong> {hero.race.charAt(0).toUpperCase() + hero.race.slice(1)}</p>
                    ...
                    <ConfirmationDialog
                        ...
                        message="Estás seguro que quieres eliminiar el superhéroe?"
